...
class RobotDemo : public SimpleRobot //all electrical components declared here
{
	...
	const static float autonomousSlowDriveSpeedMultiplier = 0.5; // Smaller numbers = higher speed
	const static float autonomousFastDriveSpeedMultiplier = 0.25; // Smaller numbers = higher speed
	const static float justIncaseMyPolarityDunGoofed = 1; // Change to -1 if everything is flipped incorrectly
...
public: //all ports declared here
	...
	{
		leftFront.Set(justIncaseMyPolarityDunGoofed*(-1 + (autonomousSlowDriveSpeedMultiplier))); // Probably needs to be negative
		leftBack.Set(justIncaseMyPolarityDunGoofed*(-1 + (autonomousSlowDriveSpeedMultiplier))); // Probably needs to be negative
		rightFront.Set(justIncaseMyPolarityDunGoofed*(-1 + (autonomousSlowDriveSpeedMultiplier))); // Probably needs to be negative
		rightBack.Set(justIncaseMyPolarityDunGoofed*(-1 + (autonomousSlowDriveSpeedMultiplier))); // Probably needs to be negative
		Wait(0.75);
		leftFront.Set(0);
	...
	{
		leftFront.Set(justIncaseMyPolarityDunGoofed*(1 - (autonomousFastDriveSpeedMultiplier))); // Probably needs to be positive
		leftBack.Set(justIncaseMyPolarityDunGoofed*(1 - (autonomousFastDriveSpeedMultiplier))); // Probably needs to be positive
		rightFront.Set(justIncaseMyPolarityDunGoofed*(1 - (autonomousFastDriveSpeedMultiplier))); // Probably needs to be positive
		rightFront.Set(justIncaseMyPolarityDunGoofed*(1 - (autonomousFastDriveSpeedMultiplier))); // Probably needs to be positive
		Wait(1);
		leftFront.Set(0);
		leftBack.Set(0);
